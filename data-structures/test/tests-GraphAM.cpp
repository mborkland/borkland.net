...
TEST_CASE("GraphAM can be constructed using GraphBuilder", "[GraphAM]")
{
    ...
    {
        auto graph = BasicGraphBuilder<>{}.weighted().build_adj_matrix();
        REQUIRE(!graph.directed());
        REQUIRE(!graph.has_satellite_data());
        REQUIRE(!graph.labeled());
        REQUIRE(graph.weighted());
    ...
    {
        auto graph = BasicGraphBuilder<>{}.use_satellite_data().build_adj_matrix();
        REQUIRE(!graph.directed());
        REQUIRE(graph.has_satellite_data());
        REQUIRE(!graph.labeled());
        REQUIRE(!graph.weighted());
    ...
    {
        auto graph = BasicGraphBuilder<>{}.directed().build_adj_matrix();
        REQUIRE(graph.directed());
        REQUIRE(!graph.has_satellite_data());
        REQUIRE(!graph.labeled());
        REQUIRE(!graph.weighted());
    ...
    {
        auto graph = LabeledGraphBuilder<>{}.build_adj_matrix();
        REQUIRE(!graph.directed());
        REQUIRE(!graph.has_satellite_data());
        REQUIRE(graph.labeled());
        REQUIRE(!graph.weighted());
    ...
    {
        auto graph = BasicGraphBuilder<>{}.directed().weighted().build_adj_matrix();
        REQUIRE(graph.directed());
        REQUIRE(!graph.has_satellite_data());
        REQUIRE(!graph.labeled());
        REQUIRE(graph.weighted());
...
create_random_graph(std::size_t num_vertices, bool directed = false, bool weighted = false)
{
    ...
    auto graph = builder.build_adj_matrix();
    ...
    std::mt19937 mt{rd()};
    std::uniform_int_distribution<> weight{1, static_cast<int>(num_vertices)};
...
void add_vertices_with_init_list(std::pair<GraphAM<>, std::vector<std::unordered_map<std::size_t, int>>>& graph_pair,
                                 std::size_t num_vertices, std::size_t vertices_to_add, bool directed = false, bool weighted = false)
{
    ...
    std::mt19937 mt{rd()};
    std::uniform_int_distribution<> weight{1, static_cast<int>(total_vertices)};
...
TEST_CASE("Vertices and edges can be added to GraphAMs in various ways", "[GraphAM]")
{
    SECTION("Vertices and edges can be added to undirected GraphAMs in various ways")
    {
    ...
    SECTION("Vertices and edges can be added to directed GraphAMs in various ways")
    {
        SECTION("Edges can be added manually to existing vertices")
        {
            constexpr std::size_t num_vertices = 1000;
    ...
    SECTION("Vertices and edges can be added to weighted GraphAMs in various ways")
    {
        SECTION("Edges can be added manually to existing vertices")
        {
            constexpr std::size_t num_vertices = 1000;
    ...
    SECTION("Vertices and edges can be added to weighted and directed GraphAMs in various ways")
    {
        SECTION("Edges can be added manually to existing vertices")
        {
            constexpr std::size_t num_vertices = 1000;
...
GraphAM<> create_small_graph(bool directed = false, bool weighted = false, bool has_satellite_data = false)
{
...
GraphAM<std::string> create_small_labeled_graph(bool directed = false, bool weighted = false, bool has_satellite_data = false)
{
...
TEST_CASE("Vertices can be removed from GraphAMs", "[GraphAM]")
{
    SECTION("Vertices can be removed from undirected GraphAMs")
    {
    ...
    SECTION("Vertices can be removed from directed GraphAMs")
    {
        SECTION("Vertices can be removed by key")
        {
    ...
    SECTION("Vertices can be removed from weighted GraphAMs")
    {
        SECTION("Vertices can be removed by key")
        {
    ...
    SECTION("Vertices can be removed from directed and weighted GraphAMs")
    {
        SECTION("Vertices can be removed by key")
        {
...
TEST_CASE("Edges can be removed from GraphAMs", "[GraphAM]")
{
    SECTION("Edges can be removed from undirected GraphAMs")
    {
        auto graph = create_small_graph();
    ...
    SECTION("Edges can be removed from directed GraphAMs")
    {
        auto graph = create_small_graph(true);
        graph.remove_edge(1, 5);
        graph.remove_edge(2, 5);
...
TEST_CASE("GraphAMs can be subscripted", "[GraphAM]")
{
    SECTION("GraphAMs can be subscripted by key")
    {
        SECTION("Non-const GraphAMs can be subscripted by key")
        {
        ...
        SECTION("Const GraphAMs can be subscripted by key")
        {
            auto graph = create_small_graph(false, false, true);
    ...
    SECTION("GraphAMs can be subscripted by label")
    {
        SECTION("Non-const GraphAMs can be subscripted by label")
        {
        ...
        SECTION("Const GraphAMs can be subscripted by label")
        {
            auto graph = create_small_labeled_graph();
            const auto& graph_ref = graph;
...
TEST_CASE("Edge weight function returns the weight between two edges for a GraphAM", "[GraphAM]")
{
...
TEST_CASE("GraphAMs can be searched using breadth-first search", "[GraphAM]")
{
    ...
    SECTION("BFS returns correct data for an undirected GraphAM")
    {
        auto graph = create_small_graph();
    ...
    SECTION("BFS returns correct data for a directed GraphAM")
    {
        auto graph = create_small_graph(true);
        auto bfs_data = graph.bfs(1);
        REQUIRE(bfs_data[1].parent == std::numeric_limits<std::size_t>::max());
...
TEST_CASE("GraphAMs can be searched using depth-first search", "[GraphAM]")
{
    ...
    SECTION("DFS returns correct parent data for an undirected graph")
    {
        ...
        REQUIRE(dfs_data[6].parent == 3);
        REQUIRE(dfs_data[9].parent == 6);
        REQUIRE(dfs_data[8].parent == 9);
    }
...
bool graphs_are_equal(const GraphAM<>& graph1, const GraphAM<>& graph2, std::size_t num_vertices)
{
...
TEST_CASE("GraphAMs are copy-constructible", "[GraphAM]")
{
    constexpr std::size_t num_vertices = 1000;
...
TEST_CASE("GraphAMs are move-constructible", "[GraphAM]")
{
    constexpr std::size_t num_vertices = 1000;
...
TEST_CASE("GraphAMs are copy-assignable", "[GraphAM]")
{
    auto orig_graph = BasicGraphBuilder<>{}.build_adj_matrix();
...
TEST_CASE("GraphAMs are move-assignable", "[GraphAM]")
{
    auto orig_graph = BasicGraphBuilder<>{}.build_adj_matrix();
    orig_graph.add_vertex();
    orig_graph.add_vertex();
...
struct A
{
    ...
    friend bool operator!=(const A& lhs, const A& rhs) { return !operator==(lhs, rhs); }
};
...
TEST_CASE("GraphAMs can hold satellite data")
{
    ...
    for (int i = 0; i < 10; ++i) {
        REQUIRE(graph[static_cast<std::size_t>(i)].data() == A{i, i + 1});
    }

    graph.remove_vertex(0);
    for (int i = 0; i < 9; ++i) {
        REQUIRE(graph[static_cast<std::size_t>(i)].data() == A{i + 1, i + 2});
    }
}
...
TEST_CASE("GraphAMs can use non-arithmetic weight functions if default_weight is specialized")
{
    ...
    for (int i = 0; i < 10; ++i) {
        for (const auto& neighbor : graph.neighbors(static_cast<std::size_t>(i))) {
            REQUIRE(neighbor.second == A{i, static_cast<int>(neighbor.first)});
        }
    }
}

TEST_CASE("GraphAM size, empty and clear functions work as expected", "[GraphAM]")
{
    SECTION("GraphAM size function returns the correct size")
    {
    ...
    SECTION("GraphAM empty function returns the correct boolean value")
    {
        auto graph = BasicGraphBuilder<>{}.build_adj_matrix();
        REQUIRE(graph.empty());
        graph.add_vertex();
    ...
    SECTION("GraphAM clear function successfully clears the graph")
    {
        auto graph = create_small_graph();
        graph.clear();
        REQUIRE(graph.empty());
